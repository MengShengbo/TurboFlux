...
import { discoverModelPresets } from '@turboflux/models/modelDiscovery'
import { maskedApiKey } from '@turboflux/contracts/apiKeyPresentation'
import { getModelReasoningCapabilities } from '@turboflux/models/modelRegistry'
import type { GitDiffScope } from '@turboflux/tools/gitService'
import { loadProfile, PERSONA_DEFINITIONS, saveProfile } from '@turboflux/models/profile'
import { loadMcpSettings, saveProjectMcpSettings } from '@turboflux/extensions/mcp/settings'
import type { McpClient } from '@turboflux/extensions/mcp/client'
import type { McpServerConfig, McpSettings } from '@turboflux/extensions/mcp/types'
import { createAgentRuntime, type AgentRuntime } from '@turboflux/agent-runtime/runtime/agentRuntime'
import { emitStreamTimingTrace, streamTimingTraceEnabled, summarizeTimings } from '@turboflux/platform/streamTimingTrace'
import type { SubAgentTaskSnapshot, SubAgentTranscriptRecord } from '@turboflux/agent-runtime/runtime/subAgentTaskManager'
import type { SubAgentResult } from '@turboflux/agent-runtime/subAgent'
import type { SubAgentEvent, SubAgentEvidence } from '@turboflux/contracts/subAgentTypes'
import type { WorkflowInstanceState, WorkflowProgressUpdate, WorkflowRunContract } from '@turboflux/contracts/workflowSurfaceTypes'
import type { PluginWorkflow } from '@turboflux/contracts/pluginTypes'
import {
...
} from '@turboflux/conversations/conversations/index'
import {
  deleteConversationAsync,
  getConversationsDir,
  sameWorkspacePath,
  updateConversationMetadata,
} from '@turboflux/conversations/conversations/store'
import { conversationV2IdFactory } from '@turboflux/conversations/conversations/conversationV2Ids'
...
} from '@turboflux/automations/automationService'
import type { AutomationCapabilityPolicy, AutomationObjective, AutomationRunMode, AutomationRunResult } from '@turboflux/automations/automationTypes'
import { classifyAutomationToolEffect, summarizeAutomationToolTarget } from '@turboflux/automations/automationSideEffects'
import { ArtifactService, type ArtifactSource } from '../artifacts/artifactService'
import { PluginService } from '@turboflux/extensions/plugins/pluginService'
import {
...
import {
  ...
  redactComputerTurns,
} from '@turboflux/conversations/privacy/computerPrivacy'
...
export class WorkbenchRuntime {
  ...
  private readonly conversationRuntimeLoads = new Map<string, Promise<WorkbenchConversationRuntime>>()
  ...
  getSnapshot(): WorkbenchSnapshot {
    ...
    const modelUsage = this.conversations.getModelUsageSummary()
  ...
  controlWorkStep(taskId: string, action: import('@turboflux/contracts/workExecutionTypes').WorkStepControlAction) {
    this.assertAvailable()
  ...
  async installPlugin(path: string, approvedPermissions: import('@turboflux/contracts/pluginTypes').PluginPermission[]) {
  ...
  private async ensureConversationRuntime(id: string): Promise<WorkbenchConversationRuntime> {
    this.assertAvailable()
    if (this.deletingConversations.has(id)) throw new Error('Conversation is being deleted')
    const pending = this.conversationRuntimeLoads.get(id)
    if (pending) return pending
    const existing = this.conversationRuntimes.get(id)
    if (existing) return existing
    // Publish the shared promise before any initialization code can reenter.
    const load = Promise.resolve().then(() => this.loadConversationRuntime(id))
    this.conversationRuntimeLoads.set(id, load)
    try {
      return await load
    } finally {
      if (this.conversationRuntimeLoads.get(id) === load) this.conversationRuntimeLoads.delete(id)
    }
  }

  private async loadConversationRuntime(id: string): Promise<WorkbenchConversationRuntime> {
    this.assertAvailable()
    if (this.deletingConversations.has(id)) throw new Error('Conversation is being deleted')
    ...
    try {
      ...
      this.assertAvailable()
      if (this.deletingConversations.has(id)) throw new Error('Conversation is being deleted')
      if (!conversation) throw new Error(`Conversation not found in this workspace: ${id}`)
      ...
      if (this.conversationRuntimes.get(id) === runtime) this.conversationRuntimes.delete(id)
  ...
  async deleteConversation(id: string): Promise<boolean> {
    this.assertAvailable()
    if (this.deletingConversations.has(id)) throw new Error('Conversation is already being deleted')
    this.deletingConversations.add(id)
    try {
      // A loader observes the deletion marker before registering or launching.
      await this.conversationRuntimeLoads.get(id)?.catch(() => undefined)
      this.assertAvailable()
      ...
      const stored = this.conversationRepositoryV2?.projection(id).conversation
      const archived = Boolean(stored && stored.status !== 'archived')
      if (stored && archived) {
        const at = Date.now()
        this.conversationRepositoryV2!.append([{
          eventId: conversationV2IdFactory(id).stable('archive', id, at),
          profileId: stored.profileId, conversationId: id, workspaceId: stored.workspaceId ?? undefined,
      ...
      if (deleted || archived || target) {
        this.conversationCatalog.remove(id)
        this.emitSnapshot()
      }
      return deleted || archived || Boolean(target)
    } finally {
    ...
    if (target) {
      ...
      if (requestedTitle) {
        ...
        if (stored && stored.status !== 'archived') {
          this.conversationRepositoryV2!.append([{
            eventId: conversationV2IdFactory(id).stable('rename', id, requestedTitle, updatedAt),
            profileId: stored.profileId, conversationId: id, workspaceId: stored.workspaceId ?? undefined,
            source: 'user', provenance: 'live', type: 'conversation.renamed', at: updatedAt,
            payload: { title: requestedTitle, titleSource: source },
          }])
          renamed = true
  ...
  destroy(): Promise<void> {
    ...
    this.destroyed = true
    this.destroyPromise = Promise.resolve().then(() => this.dispose())
  ...
  private async dispose(): Promise<void> {
    ...
    // Stop all admitted work before waiting for loading or external resources.
    await Promise.allSettled([
      ...this.conversationRuntimeLoads.values(),
      ...[...this.conversationRuntimes.values()].map(runtime => this.destroyConversationRuntime(runtime)),
    ])
  ...
  private async runPrompt(slot: WorkbenchConversationRuntime, prompt: string, attachments: AgentAttachment[] | undefined, capabilities: AgentCapabilitySelection | undefined, inputId: string, reuseLastUserTurn = false): Promise<void> {
    ...
    try {
      ...
      const completedRun = slot.runtime.engine.getWorkExecutionSnapshot().runs.find(run => run.id === inputId)
      this.publishConversationEvents(slot, slot.work.finishRun({
        outcome: completionStatus,
        error: errorMessage,
        run: completedRun,
        state: {
          ...slot.runtime.engine.getRunState(),
          phase: completionStatus === 'failed' ? 'recoverable_error' : 'completed',
          updatedAt: completedRun?.completedAt ?? Date.now(),
        },
        at: completedRun?.completedAt,
      }))
      this.emit({
      ...
      slot.updatedAt = Date.now()
      // settlePromptRun publishes the snapshot after releasing run ownership.
      // Publishing here would pair a terminal event with status "running".
    }
  }
  ...
  private handleAgentEvent(slotOrEvent: WorkbenchConversationRuntime | AgentEventType, receivedEvent?: AgentEventType): void {
    ...
    if (
      event.type === 'run:state' && !['completed', 'recoverable_error'].includes(event.state.phase)
  ...
  private publishConversationEvents(slot: WorkbenchConversationRuntime, events: readonly import('@turboflux/conversations/events/index').AnyConversationEvent[]): void {
  ...
  private async ensureAutomationConversation(claim: AutomationClaim): Promise<WorkbenchConversationRuntime> {
    ...
    if (existingId) {
      try {
        return await this.ensureConversationRuntime(existingId)
      } catch (error) {
        if (!(error instanceof Error) || error.message !== `Conversation not found in this workspace: ${existingId}`) throw error
      }
    }
  ...
  private destroyConversationRuntime(slot: WorkbenchConversationRuntime): Promise<void> {
    if (slot.destroyPromise) return slot.destroyPromise
    slot.destroying = true
    slot.destroyPromise = Promise.resolve().then(() => this.disposeConversationRuntime(slot))
    return slot.destroyPromise
  }

  private async disposeConversationRuntime(slot: WorkbenchConversationRuntime): Promise<void> {
    try { slot.runtime.engine.abort() } catch {}
    ...
    await waitForSettlement(
      this.plugins.detachMcpClient(slot.runtime.mcpClient),
      RESOURCE_SHUTDOWN_TIMEOUT_MS,
      'Plugin detachment timed out',
    ).catch(() => undefined)
