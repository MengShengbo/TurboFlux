...
import type { AgentAttachment } from '@turboflux/contracts/agentTypes'
...
export class McpClient {
  ...
  async connect(name: string, config: McpServerConfig): Promise<McpConnection> {
    ...
    try {
      ...
      const discovered = await withTimeout((async () => {
        ...
        const tools = await this.discoverTools(name, client, config, instructions, startup.signal)
        return { instructions, tools }
      })(), normalizeTimeout(config.startupTimeoutMs, 10_000), `MCP server "${name}" startup timed out`)
      conn.instructions = discovered.instructions
      conn.tools = discovered.tools
  ...
  private async discoverTools(serverName: string, client: Client, config: McpServerConfig, instructions?: string, signal?: AbortSignal): Promise<McpToolInfo[]> {
