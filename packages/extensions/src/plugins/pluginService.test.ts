...
import { getSubAgentDefinition, loadDynamicAgents } from '@turboflux/agent-runtime/subAgent'
...
describe('PluginService', () => {
  it('installs, enables, projects Skills, disables, and persists metadata', async () => {
    ...
    const agents = loadDynamicAgents(workspace)
    expect(getSubAgentDefinition('example_plugin_researcher', agents)?.allowedTools).toEqual(['web_search', 'web_fetch', 'write_research_report'])
    expect(getSubAgentDefinition('example_plugin_researcher', agents)).toMatchObject({
      maxTurns: 12,
    ...
    agents.reload(workspace)
    expect(getSubAgentDefinition('example_plugin_researcher', agents)).toBeUndefined()
