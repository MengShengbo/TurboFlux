...
import type { ConversationDraftState, PersistedConversation } from '@turboflux/conversations/conversations/types'
...
import { normalizeConversationV2Id, stableConversationV2Id } from '@turboflux/conversations/conversations/conversationV2Ids'
import type { AnyAppendConversationEventV2Input, AnyConversationEventV2 } from '@turboflux/conversations/conversations/conversationV2Types'
...
export class ProfileArchiveImporter {
  ...
  async execute(input: {
    ...
    const scan = await scanProfileArchive({ path: input.plan.sourcePath, password: input.password, signal: input.signal })
