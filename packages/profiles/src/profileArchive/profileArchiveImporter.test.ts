...
import { isConversationV2Uuid, normalizeConversationV2Id } from '@turboflux/conversations/conversations/conversationV2Ids'
...
describe('profile archive transactional importer', () => {
  ...
  it.each(['scan', 'extraction'] as const)('does not commit and removes staging when authentication fails during %s', async phase => {
    ...
    const plan = importer.plan(archive.path, preview, { archiveId: preview.archiveId, displayName: 'Damaged', selectedComponents: ['profile.preferences'] })
    const before = registry.snapshot()
    const filesBefore = readdirSync(registry.profilesRoot)
    const damageTag = () => {
      const bytes = readFileSync(archive.path)
      bytes[bytes.length - 1] = bytes[bytes.length - 1]! ^ 0xff
      writeFileSync(archive.path, bytes)
    }
    if (phase === 'scan') damageTag()
    await expect(importer.execute({
      plan, password: archive.password,
      onProgress: current => { if (phase === 'extraction' && current === 'staging') damageTag() },
    })).rejects.toMatchObject({ code: 'ARCHIVE_AUTHENTICATION_FAILED' })
    expect(registry.snapshot()).toEqual(before)
    expect(readdirSync(registry.profilesRoot)).toEqual(filesBefore)
    expect(existsSync(importer.journalRoot) ? readdirSync(importer.journalRoot) : []).toEqual([])
  })
  ...
  it.each(['staging', 'committing'] as const)('rolls back when cancelled during %s', async phase => {
    ...
    const importer = new ProfileArchiveImporter({ registry })
    const preview = await importer.inspect(archive.path, archive.password)
    const plan = importer.plan(archive.path, preview, { archiveId: preview.archiveId, displayName: 'Cancelled', selectedComponents: ['profile.preferences'] })
    const before = registry.snapshot()
    const filesBefore = readdirSync(registry.profilesRoot)
    const controller = new AbortController()
    const reason = new Error('Import cancelled')
    await expect(importer.execute({
      plan, password: archive.password, signal: controller.signal,
      onProgress: current => { if (current === phase) controller.abort(reason) },
    })).rejects.toBe(reason)
    expect(registry.snapshot()).toEqual(before)
    expect(readdirSync(registry.profilesRoot)).toEqual(filesBefore)
