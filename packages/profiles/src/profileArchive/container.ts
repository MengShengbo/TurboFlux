...
export async function writeProfileArchive(input: {
  ...
  try {
    ...
    if (input.verifyDocument) await verifyProfileArchive({ path: temporaryPath, password: secret, signal: input.signal })
    else await verifyProfileArchiveContainer({ path: temporaryPath, password: secret, signal: input.signal })
...
async function readEnvelope(path: string, limits: ProfileArchiveReadLimits): Promise<ParsedEnvelope> {
  ...
  try {
    ...
    if (payloadBigInt === 0n) throw new Error('Archive payload is empty')
...
export async function readProfileArchive(input: {
  ...
  try {
    ...
    let decipher: ReturnType<typeof createArchiveDecipher> | undefined
    if (envelope.encrypted) {
      key = await deriveArchiveKey(secret!, envelope.header)
      input.signal?.throwIfAborted()
      decipher = createArchiveDecipher(key, envelope.header, envelope.aad, envelope.trailer)
    } else {
      const hash = createHash('sha256')
      for await (const chunk of createReadStream(input.path, { start: envelope.payloadOffset, end: payloadEnd, signal: input.signal })) hash.update(chunk as Buffer)
    ...
    const consume = async (decoded: AsyncIterable<Buffer>): Promise<void> => {
      const reader = new StreamByteReader(decoded[Symbol.asyncIterator](), limits, envelope.payloadLength)
      while (true) {
    ...
    // Own the consumer as well as every stream; success must include final GCM authentication.
    if (decipher) await pipeline(source, decipher, gunzip, consume, { signal: input.signal })
    else await pipeline(source, gunzip, consume, { signal: input.signal })
    input.signal?.throwIfAborted()
