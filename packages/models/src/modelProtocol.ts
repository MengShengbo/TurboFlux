import type { APIConfig } from '@turboflux/contracts/stateTypes'
...
const PROTOCOL_MISMATCH_DETAIL = new RegExp([
  ...
  '.{0,100}',
  '(?:endpoint|route|path|url|schema|payload|request format|model|messages|instructions|input|max_tokens|max_completion_tokens|max_output_tokens|anthropic-version|x-api-key|authorization|content-type|tool_choice|tools|stream|thinking|output_config|cache_control|prompt_cache_key|prompt_cache_options|prompt_cache_retention)',
  '|(?:endpoint|route|path|url|schema|payload|request format|model|messages|instructions|input|max_tokens|max_completion_tokens|max_output_tokens|anthropic-version|x-api-key|authorization|content-type|thinking|output_config|cache_control|prompt_cache_key|prompt_cache_options|prompt_cache_retention)',
  '.{0,100}',
...
function planUnfilteredModelProtocols(provider: APIConfig['provider'], model: string): ModelProtocol[] {
  // Custom endpoints are generally OpenAI-compatible gateways even when the
  // model id contains "claude" or "gpt". Provider configuration outranks
  // the model marketing name for third-party protocol selection.
