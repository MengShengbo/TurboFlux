import type { AnthropicThinkingBlock, TokenUsage } from '@turboflux/contracts/agentTypes'
import {
  BoundedStreamBuffer,
...
export class AnthropicStreamParser {
  ...
  handleLine(line: string): void {
    ...
    try { event = JSON.parse(json) as Record<string, any> } catch {
      if (this.pendingSseEventType === 'error') {
        this.streamFailure = 'Anthropic stream returned a malformed error event'
    ...
    this.pendingSseEventType = ''
    if (event && typeof event === 'object') this.handleEvent(eventType, event)
  }
  ...
  private handleEvent(eventType: string, event: Record<string, any>): void {
    ...
    if (eventType === 'error') {
      ...
      this.streamFailure = typeof providerError?.message === 'string' && providerError.message.trim()
        ? providerError.message.trim()
        : typeof event.message === 'string' && event.message.trim()
          ? event.message.trim()
          : typeof providerError?.type === 'string' && providerError.type.trim()
            ? `Anthropic stream error: ${providerError.type.trim()}`
  ...
  private handleMessageDelta(event: Record<string, any>): void {
    ...
    if (Number.isSafeInteger(event.usage.output_tokens) && event.usage.output_tokens >= 0) this.usage.output = this.outputTokens = event.usage.output_tokens
    if (typeof event.usage.cache_read_input_tokens === 'number') {
      this.usage.cached = this.cacheReadTokens = event.usage.cache_read_input_tokens
    }
    if (typeof event.usage.cache_creation_input_tokens === 'number') {
      this.usage.cacheWrite = this.cacheCreationTokens = event.usage.cache_creation_input_tokens
    }
    this.emitUsage()
  }

  private handleMessageStart(event: Record<string, any>): void {
    if (typeof event.message?.id === 'string') this.callbacks.onResponseId?.(event.message.id)
    if (!event.message?.usage) return
    const usage = event.message.usage
    if (Number.isSafeInteger(usage.input_tokens) && usage.input_tokens >= 0) {
      this.inputTokens = usage.input_tokens
      this.reportedInput = true
    }
    if (Number.isSafeInteger(usage.cache_read_input_tokens) && usage.cache_read_input_tokens >= 0) this.usage.cached = this.cacheReadTokens = usage.cache_read_input_tokens
    if (Number.isSafeInteger(usage.cache_creation_input_tokens) && usage.cache_creation_input_tokens >= 0) this.usage.cacheWrite = this.cacheCreationTokens = usage.cache_creation_input_tokens
    if (Number.isSafeInteger(usage.output_tokens) && usage.output_tokens >= 0) this.usage.output = this.outputTokens = usage.output_tokens
    this.emitUsage()
  }

  private emitUsage(): void {
    if (this.reportedInput) this.usage.input = this.inputTokens + this.cacheReadTokens + this.cacheCreationTokens
