...
import { REASONING_EFFORTS, type ReasoningEffort } from '@turboflux/contracts/agentTypes'
import {
...
import { writeFileAtomicSync } from '@turboflux/platform/fileIO'
...
function mergeCapabilities(primary: ModelCapabilities, fallback: ModelCapabilities): ModelCapabilities {
  return {
    ...
    reasoningDefaultEffort: primary.reasoningDefaultEffort ?? fallback.reasoningDefaultEffort,
    reasoningDefaultEnabled: primary.reasoningDefaultEnabled ?? fallback.reasoningDefaultEnabled,
    reasoningMandatory: primary.reasoningMandatory ?? fallback.reasoningMandatory,
    reasoningSupportsMaxTokens: primary.reasoningSupportsMaxTokens ?? fallback.reasoningSupportsMaxTokens,
  }
}
...
function extractModelMetadata(raw: JsonRecord): ParsedModelMetadata {
  ...
  const rawDefaultEffort = cleanText(reasoning?.default_effort ?? capabilities?.default_effort, 32).toLowerCase()
  const reasoningDefaultEffort = REASONING_EFFORTS.includes(rawDefaultEffort as ReasoningEffort)
    ? rawDefaultEffort as ReasoningEffort
    : undefined
  const reasoningDefaultEnabled = typeof reasoning?.default_enabled === 'boolean'
    ? reasoning.default_enabled
    : typeof capabilities?.default_enabled === 'boolean' ? capabilities.default_enabled : undefined
  const reasoningMandatory = typeof reasoning?.mandatory === 'boolean'
    ? reasoning.mandatory
    : typeof capabilities?.mandatory === 'boolean' ? capabilities.mandatory : undefined
  const reasoningSupportsMaxTokens = typeof reasoning?.supports_max_tokens === 'boolean'
    ? reasoning.supports_max_tokens
    : typeof capabilities?.supports_max_tokens === 'boolean' ? capabilities.supports_max_tokens : undefined
