import type { ReasoningEffort } from '@turboflux/contracts/agentTypes'
...
export function setOpenAIPromptCacheLifetime(body: Record<string, unknown>, model?: string): void {
  if (/gpt-5\.6/i.test(String(model || ''))) {
    body.prompt_cache_options = { ttl: '30m' }
    return
  }
  if (/gpt-5\.5/i.test(String(model || ''))) body.prompt_cache_retention = '24h'
}
...
export function extractUnsupportedRequestParam(error?: string): string | null {
  ...
  const knownOptionalParams = [
    ...
    'tools', 'prompt_cache_key', 'prompt_cache_options.ttl', 'prompt_cache_options', 'prompt_cache_retention', 'store', 'text.verbosity', 'verbosity', 'text',
...
export function removeOpenAICompatibleRequestParam(body: Record<string, unknown>, param: string): boolean {
  ...
  const removable = new Set([
    ...
    'prompt_cache_key', 'prompt_cache_options', 'prompt_cache_retention', 'store', 'text',
  ])
