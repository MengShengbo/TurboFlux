...
describe('modelRegistry', () => {
  ...
  it('exposes provider-native effort ranges instead of TurboFlux modes', () => {
    ...
    expect(getModelReasoningCapabilities('gpt-6-astra', 'openai', { reasoning: true })?.control).toBe('effort')
    expect(getModelReasoningCapabilities('gpt-6-astra', 'openai', { reasoning: true })?.efforts).toContain('high')
  })
  ...
  it('honors gateway reasoning defaults and mandatory thinking', () => {
    ...
    expect(getModelReasoningCapabilities('vendor/claude-model', 'custom', capabilities)).toMatchObject({
      defaultEffort: 'high',
      defaultEnabled: true,
