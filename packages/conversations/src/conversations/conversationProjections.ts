import { mergeModelRequest } from '@turboflux/contracts/modelUsage'
import type {
...
export interface ConversationProjectionReducerState {
  workspaceRequired: boolean
  conversationUpdatedAt?: number
  itemAliases: Array<[string, number]>
}
...
export function projectConversationEvents(events: readonly AnyConversationEventV2[]): ConversationTranscriptProjectionV2 {
  return createConversationProjector().apply(events)
}

export function createConversationProjector(checkpoint?: ConversationProjectionCheckpoint) {
  const projection: ConversationTranscriptProjectionV2 = checkpoint?.projection ?? {
    conversation: null,
    runs: [],
    turns: [],
    items: [],
  ...
  const modelRequestById = new Map((projection.modelRequests ?? []).map(record => [record.id, record]))
  ...
  for (const [alias, index] of checkpoint?.reducer.itemAliases ?? []) itemById.set(alias, projection.items[index]!)
  let workspaceRequired = checkpoint?.reducer.workspaceRequired ?? false
  let conversationUpdatedAt = checkpoint?.reducer.conversationUpdatedAt

  function apply(events: readonly AnyConversationEventV2[]): ConversationTranscriptProjectionV2 {
    if (events.length === 0) return projection
    let previousSeq = projection.throughSeq
    // Finalization must not change the reducer's timestamp across batch boundaries.
    if (projection.conversation && conversationUpdatedAt !== undefined) projection.conversation.updatedAt = conversationUpdatedAt
    for (const event of events) {
      ...
      switch (event.type) {
        ...
        case 'model.request_updated': {
          ...
          const previous = modelRequestById.get(incoming.id)
          const merged = mergeModelRequest(previous, incoming)
          if (previous) Object.assign(previous, merged)
          else {
            ;(projection.modelRequests ??= []).push(merged)
            modelRequestById.set(merged.id, merged)
          }
          break
        }
        case 'turn.started': {
        ...
        case 'turn.completed': {
          ...
          if (turn && event.payload.metadata) turn.metadata = clone(event.payload.metadata)
          break
        }
        case 'item.created': {
    ...
    conversationUpdatedAt = projection.conversation?.updatedAt
    if (projection.conversation) {
  ...
  function snapshot(): ConversationProjectionCheckpoint {
    const positions = new Map(projection.items.map((item, index) => [item, index]))
    return {
      projection,
      reducer: {
        workspaceRequired,
        conversationUpdatedAt,
        itemAliases: [...itemById].filter(([id, item]) => id !== item.id).map(([id, item]) => [id, positions.get(item)!]),
      },
    }
  }

  return { apply, snapshot, projection }
}
