...
import { ConversationRuntimeRepositoryV2, persistedConversationFromProjectionV2 } from './conversationRuntimeRepositoryV2'
...
import type { ToolResult } from '@turboflux/contracts/agentTypes'
import type { ConversationTimelineEntryV2 } from './conversationV2Types'
...
describe('ConversationRuntimeRepositoryV2', () => {
  it.each([{}, { runId: 'run' }, { turnId: 'turn' }, { runId: 'run', turnId: 'turn' }])('preserves replay order for repeated events and approval coordinates %j', coordinates => {
    const directory = root()
    const runtime = new ConversationRuntimeRepositoryV2(directory, 'profile-1', 'workspace-12345678', '/source/project', () => 100)
    runtime.persist(conversation())
    const projection = new ConversationRepositoryV2(directory).projection('conversation-1')
    const event = (type: ConversationTimelineEntryV2['type'], seq: number, extra = coordinates): ConversationTimelineEntryV2 => ({ type, seq, eventId: `e-${seq}`, at: seq, ...extra })
    projection.turns = [{ id: 'turn', conversationId: 'conversation-1', role: 'assistant', status: 'completed', createdAt: 1, completedAt: 11 }]
    projection.runs = []
    projection.items = [
      { schemaVersion: 1, id: 'message', conversationId: 'conversation-1', turnId: 'turn', kind: 'assistant_message', status: 'completed', createdAt: 4, updatedAt: 4, payload: { text: 'Recorded answer' } },
      { schemaVersion: 1, id: 'approval', conversationId: 'conversation-1', ...coordinates, kind: 'approval', status: 'completed', createdAt: 3, updatedAt: 9, payload: { requestId: 'request', requestKind: 'permission', question: 'Allow?', decision: 'allow-once' } },
    ]
    projection.timeline = [
      event('turn.started', 1, { turnId: 'turn' }),
      event('approval.resolved', 2),
      event('approval.requested', 3),
      { ...event('item.created', 4), itemId: 'message' },
      event('turn.started', 5, { turnId: 'turn' }),
      event('turn.completed', 6, { turnId: 'turn' }),
      event('approval.requested', 7),
      event('approval.resolved', 9),
      { ...event('item.created', 10), itemId: 'message' },
      event('turn.completed', 11, { turnId: 'turn' }),
    ]
    projection.throughSeq = 11
    const loaded = persistedConversationFromProjectionV2(projection, '/source/project')!
    expect(loaded.canonicalEvents?.map(event => event.type)).toEqual([
      'turn.started', 'approval.requested', 'stream.committed', 'approval.resolved', 'turn.completed',
    ])
    expect(loaded.turns).toMatchObject([{ id: 'turn', content: 'Recorded answer' }])
    const started = loaded.canonicalEvents?.find(event => event.type === 'turn.started')
    loaded.turns[0]!.content = 'Edited after loading'
    expect(started?.payload.turn.content).toBe('Recorded answer')
    expect(projection.items[0]?.payload).toEqual({ text: 'Recorded answer' })
  })

  it.each(['history', 'live', 'canonical'] as const)('preserves task mode and multiple stop records through %s persistence', source => {
    const directory = root()
    ...
    if (source === 'history') runtime.persist(value)
      ...
      if (source === 'canonical') {
        runtime.appendCanonical({ ...base, seq: 2, eventId: 'timing', at: 248_000, type: 'run.completed', payload: { outcome: 'completed', run: value.workExecution.runs[0] } }, value)
      } else {
