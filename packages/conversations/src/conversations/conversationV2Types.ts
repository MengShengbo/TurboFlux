import type { AgentMode, AgentTurn, ApprovalPolicy, ModelRequestRecord, ToolResult } from '@turboflux/contracts/agentTypes'
import type { ResponseMode, WorkExecutionSegment } from '@turboflux/contracts/workExecutionTypes'
...
export interface ConversationTurnV2 {
  ...
  createdAt: number
  completedAt?: number
  metadata?: Pick<NonNullable<AgentTurn['metadata']>, 'tokens' | 'model' | 'duration' | 'modelRequestId' | 'modelAttemptId' | 'internal' | 'internalKind'>
}
...
export interface ConversationEventPayloadMapV2 {
  ...
  'turn.completed': { completedAt: number; interrupted?: boolean; metadata?: ConversationTurnV2['metadata'] }
