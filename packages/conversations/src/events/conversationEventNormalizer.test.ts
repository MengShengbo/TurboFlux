...
import type { AgentSession, AgentTurn, ToolCall, ToolResult } from '@turboflux/contracts/agentTypes'
...
describe('ConversationEventNormalizer', () => {
  it('normalizes an ordinary send into one ordered run, turn, step, stream, and terminal lifecycle', () => {
    ...
    log.appendMany(normalizer.finishRun({ outcome: 'completed', at: 8 }))
  ...
  it('leaves failure settlement to the run owner and publishes it exactly once', () => {
    const normalizer = new ConversationEventNormalizer('conversation-1')
    const log = new ConversationEventLog('conversation-1')
    log.appendMany(normalizer.startRun({ runId: 'run-1' }))
    ...
    expect(log.getEvents().filter(event => event.type === 'run.completed')).toHaveLength(0)
    log.appendMany(normalizer.finishRun({ outcome: 'failed', error: 'network failed' }))
    log.appendMany(normalizer.finishRun({ outcome: 'failed', error: 'duplicate failure' }))
  ...
  it('never regresses a completed run back to a running phase', () => {
    ...
    log.appendMany(normalizer.finishRun({ outcome: 'completed' }))
    append(log, normalizer.normalizeAgent({ type: 'run:state', state: { phase: 'completed', updatedAt: 10 } }))

    expect(log.getEvents().map(event => event.type)).toEqual(['run.started', 'run.completed'])
  })
