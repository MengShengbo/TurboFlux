...
import type { RuntimeTaskEvent } from '@turboflux/contracts/runtimeTaskTypes'
...
describe('RuntimeTaskManager', () => {
  ...
  it('shares a pending stop across direct and runtime-wide cancellation', async () => {
    const manager = new RuntimeTaskManager()
    let release!: () => void
    const gate = new Promise<void>(resolve => { release = resolve })
    const stop = vi.fn(() => gate)
    const task = manager.createTask({ kind: 'shell', status: 'running' }, { stop })
    const first = manager.stopTask(task.id)
    const second = manager.stopTask(task.id)
  ...
  it('cancels other tasks without waiting for a slow task to stop', async () => {
    const manager = new RuntimeTaskManager()
    let release!: () => void
    const gate = new Promise<void>(resolve => { release = resolve })
    manager.createTask({ kind: 'shell', status: 'running' }, { stop: () => gate })
    const stopSecond = vi.fn()
    manager.createTask({ kind: 'shell', status: 'running' }, { stop: stopSecond })
    const stopping = manager.stopAll()

    try {
      await vi.waitFor(() => expect(stopSecond).toHaveBeenCalledOnce())
    } finally {
      release()
      await stopping
    }
