...
import type { Memory, MemoryGroup } from '@turboflux/contracts/memoryTypes'
...
function buildMemoriesFromBody(
  ...
  scope: Memory['scope'] = 'workspace_shared',
): Memory[] {
  const bullets = splitMarkdownIntoBullets(body)
  const relSource = sourceOverride ?? toRelative(ctx.workspacePath, absPath)
  let mtime = Date.now()
  try {
...
function loadMarkdownRuleFile(
  ...
  const items = buildMemoriesFromBody(absPath, text, [...extraTags, ...tags], ctx, sourceOverride, scope)
  if (items.length === 0) return null

  return {
    id: `grp_${memoryId(absPath, 0)}`,
    label: groupLabel,
    source: sourceOverride ?? toRelative(ctx.workspacePath, absPath),
    loader: loaderName,
    items,
  }
}
...
function loadTurbofluxFactsJsonl(
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    byId.set(id, {
      id,
      scope: (parsed.scope as Memory['scope']) || (sourceOverride ? 'workspace_private' : 'workspace_shared'),
...
export function loadAllMemoryGroups(
  workspacePath: string,
  options: { includeInactive?: boolean; privateMemoryRoot?: string } = {},
): {
  groups: MemoryGroup[]
  warnings: string[]
