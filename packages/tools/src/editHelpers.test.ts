...
describe('applyEdit', () => {
  ...
  it.each(['$&', '$$', '$`', "$'", '$1', '$<name>'])('preserves replacement marker %s literally', marker => {
    const replacement = `before ${marker} after`
    expect(applyEdit('prefix TARGET suffix', 'TARGET', replacement, false, 'source.js'))
      .toEqual({ content: `prefix ${replacement} suffix`, replacements: 1 })
    expect(applyEdit('prefix TARGET suffix', 'TARGET', replacement, true, 'source.js'))
      .toEqual({ content: `prefix ${replacement} suffix`, replacements: 1 })
  })

  it('preserves regex replacement source when moving a Markdown escaping operation', () => {
    const before = String.raw`    .replaceAll('\n', '&#10;').replace(/[\\` + '`' + String.raw`*_]/g, '\\$&');`
    const after = String.raw`    .replace(/[\\` + '`' + String.raw`*_]/g, '\\$&').replaceAll('\n', '&#10;');`
    const result = applyEdit(`function escape(value) {\n${before}\n}`, before, after, false, 'format.mjs')
    expect(result).toEqual({ content: `function escape(value) {\n${after}\n}`, replacements: 1 })
  })
