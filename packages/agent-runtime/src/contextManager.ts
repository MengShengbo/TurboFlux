...
import type { AgentAttachment, AgentTurn, TokenUsage } from '@turboflux/contracts/agentTypes'
import type { ContextHandoff, ContextSegment } from '@turboflux/contracts/stateTypes'
...
import { countMessagesTokens, countTextTokens, type TokenCountResult } from '@turboflux/models/tokenCounter'
...
export class ContextManager {
  private currentContextUsage: TokenUsage | null = null
  ...
  /**
   * Replace the current context snapshot with provider-reported values.
   * Input is the complete prompt context, output is the content appended by
   * the response, and cached is the subset of input served from cache.
   */
  updateCurrentContextUsage(inputTokens: number, outputTokens: number, cachedTokens = 0): void {
  ...
  getCurrentContextUsage(): TokenUsage {
    return this.currentContextUsage ?? { source: 'unknown' }
  }
