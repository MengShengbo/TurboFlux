...
import { generateSessionId, generateTurnId } from '@turboflux/contracts/agentTypes'
import { existsSync, statSync } from 'node:fs'
import { randomUUID } from 'node:crypto'
import type { MemoryKind, MemoryScope } from '@turboflux/contracts/memoryTypes'
import { browserToolNeedsApproval, describeBrowserPermission, describeBrowserToolActivity, isBuiltInBrowserTool } from '@turboflux/contracts/browserToolPresentation'
import { computerToolApprovalLevel, describeComputerPermission, describeComputerToolActivity, isBuiltInComputerTool } from '@turboflux/contracts/computerToolPresentation'
...
import { declaredResponseMode, RESPONSE_MODE_INSTRUCTION, RESPONSE_MODE_TOOL } from './responseMode'
import type { WorkExecutionSnapshot, WorkStepControlAction } from '@turboflux/contracts/workExecutionTypes'
...
import { toolsToOpenAIFormat, toolsToAnthropicFormat, getToolByName, getToolsForMode, validateToolArgs } from '@turboflux/tools/toolRegistry'
import { applyEdit, stripLineNumberPrefix } from '@turboflux/tools/editHelpers'
import { applyPatchAdd, applyPatchHunks, parseApplyPatch, type ApplyPatchOperation } from '@turboflux/tools/applyPatch'
import { canComputeDiff, computeHunks, summarizeHunks } from '@turboflux/presentation/diffCompute'
import { shouldAutoBackgroundCommand } from '@turboflux/tools/commandExecutionPolicy'
...
import { countMessagesTokens, countTurnishTokens } from '@turboflux/models/tokenCounter'
import { resolveNativeReasoningRequest } from '@turboflux/models/modelRegistry'
import {
...
} from '@turboflux/models/requestCompatibility'
import { TurnStrategyPlanner, type TurnStrategy } from './turnStrategy'
import { toolCallSignature } from './toolExecutionLedger'
import { createDefaultPipeline, type PermissionPipeline } from '@turboflux/tools/permissions'
import type { TerminalSessionInfo } from '@turboflux/contracts/terminalTypes'
import type { RuntimeTask, RuntimeTaskEvent, RuntimeTaskPresentation, RuntimeTaskPresentationKind } from '@turboflux/contracts/runtimeTaskTypes'
import { isMcpTool, parseMcpToolName, executeMcpTool, getMcpAgentTools, validateMcpToolArgs } from '@turboflux/extensions/mcp/toolBridge'
import type { McpClient } from '@turboflux/extensions/mcp/client'
import type { SubAgentDefinition, SubAgentEvent, SubAgentEvidence } from '@turboflux/contracts/subAgentTypes'
import type { WorkflowCheckpointSpec, WorkflowProgressUpdate, WorkflowRunContract, WorkflowSurfaceSpec } from '@turboflux/contracts/workflowSurfaceTypes'
import { resolvePath, toWorkspaceRelative } from '@turboflux/platform/pathUtils'
import { normalizeBaseUrl } from '@turboflux/models/normalizeBaseUrl'
import { createTurboFluxRequestHeaders } from '@turboflux/models/clientIdentity'
import {
...
} from '@turboflux/models/modelProtocol'
import { resolveRequestMaxTokens } from '@turboflux/models/modelRequestBudget'
import { dispatchTaskTool, type TaskSystemCreationEvent } from './taskToolDispatcher'
import { getSubAgentDefinition, runSubAgent, SubAgentRegistry, getAvailableAgentTypes } from './subAgent'
import type { ToolExecutor, WebFetchResponse, WebSearchResponse, RequestOptions, Result } from '@turboflux/contracts/toolExecutor'
import type { AgentStateProvider, APIConfig, APIModel, ContextCompactionState, ContextHandoff, ContextHandoffFacts, ContextReservoirEntry, ContextSegment, WorkspaceInfo } from '@turboflux/contracts/stateTypes'
import type { TreeNode } from '@turboflux/contracts/types'
import type { EnhancedToolDef } from '@turboflux/contracts/toolTypes'
import { parseTextToolCalls, stripTextToolCallMarkup } from '@turboflux/contracts/toolCallMarkup'
import {
  detectGitRepo,
  fetchGitDiff,
  fetchGitLog,
...
} from '@turboflux/tools/gitService'
import { hashText } from '@turboflux/platform/fileIO'
...
import { hasCompleteToolPayloads, isOutputLimitFinishReason } from '@turboflux/models/modelStream'
import { appendRuntimeContextToLatestUserMessage, normalizeAnthropicToolMessages } from '@turboflux/models/modelMessages'
import {
  ...
  redactComputerTurns,
} from '@turboflux/contracts/computerPrivacy'
import { AnthropicStreamParser } from '@turboflux/models/providers/anthropicStream'
import { OpenAIChatStreamParser } from '@turboflux/models/providers/openAIChatStream'
import { OpenAIResponsesStreamParser } from '@turboflux/models/providers/openAIResponsesStream'
import { runModelRequest } from '@turboflux/models/modelRequestOrchestrator'
import type { ToolCallBatch } from './toolCallOrchestrator'
import {
  planContextCompaction,
  projectTurnsForModelContext,
} from './contextCompactionBoundary'
import { presentRequestError } from '@turboflux/presentation/requestErrorPresentation'
import { normalizeBuiltInToolArguments } from '@turboflux/tools/toolArgumentNormalization'
import { contentSearchResult, fileSearchResult, formatRetrievalResult } from '@turboflux/tools/retrievalResults'
import type { RetrievalResult, RetrievedResource } from '@turboflux/contracts/retrievalTypes'
import type { ToolResultData } from '@turboflux/contracts/toolResultData'
import { ModelSurface } from '@turboflux/models/modelSurface'
import type { ModelSurfaceState } from '@turboflux/contracts/modelSurfaceTypes'

export {
  extractResponsesReasoningEventDelta,
  extractResponsesReasoningSummary,
} from '@turboflux/models/modelStream'
export {
  appendRuntimeContextToLatestUserMessage,
  normalizeAnthropicToolMessages,
} from '@turboflux/models/modelMessages'
...
export { downgradeReasoningEffort } from '@turboflux/models/requestCompatibility'
...
export class AgentEngine {
  ...
  private automationSubAgentPolicy: {
    runId: string
    allowedTools: string[]
    deniedTools: string[]
  ...
  setAutomationSubAgentPolicy(policy: typeof this.automationSubAgentPolicy): void {
  ...
  private modelDisabledToolNames(): string[] {
    ...
    disabledTools.delete(RESPONSE_MODE_TOOL)
    if (getAvailableAgentTypes(this.agentRegistry).length === 0) disabledTools.add('spawn_agent')
  ...
  retrySubAgentTask(taskId: string): RuntimeTask {
    ...
    const definition = getSubAgentDefinition(previous.agentType, this.agentRegistry)
  ...
  async run(userMessage: string, options?: { reuseLastUserTurn?: boolean; attachments?: NonNullable<AgentTurn['metadata']>['attachments']; capabilities?: NonNullable<AgentTurn['metadata']>['capabilities']; userTurnId?: string; workflowContext?: string; workflow?: WorkflowRunContract; onWorkflowProgress?: (update: WorkflowProgressUpdate) => void }): Promise<AgentTurn[]> {
    ...
    try {
      ...
      let toolRounds = 0
      let responseModeDeclarationExempted = false
      ...
      while (true) {
        ...
        if ((!assistantTurn.toolCalls || assistantTurn.toolCalls.length === 0) && !assistantTurn.content.trim()) {
          if (this.finalDeliveryRetryAttempts === 0) {
            ...
            const currentUserTurn = [...this.session.turns].reverse().find(turn => turn.role === 'user' && turn.metadata?.workRunId === workRunId)
        ...
        if (!responseModeDeclarationExempted && assistantTurn.toolCalls.every(call => call.name === RESPONSE_MODE_TOOL)) {
          responseModeDeclarationExempted = true
        } else {
          toolRounds += 1
        }
...
5. After fixing the approach, re-attempt with corrected parameters
  ...
  private async callModel(): Promise<AgentTurn> {
    ...
    this.declaringResponseMode = this.needsResponseMode()
    let automaticModeDeclaration = false
    try {
      const turn = await runModelRequest({
        ...
        invoke: async protocol => {
          automaticModeDeclaration = protocol === 'openai_responses' && this.declaringResponseMode && !this.forceResponseModeDeclaration
          if (protocol === 'anthropic_messages') {
      ...
      if (this.declaringResponseMode) {
        if (!turn.metadata?.interrupted && !turn.metadata?.internalError) {
          try {
            declaredResponseMode(turn)
          } catch (error) {
            if (!automaticModeDeclaration) throw error
            // Validate before executing any proposed tools. Only a model that
            // violates the declaration contract needs a forced retry.
            this.forceResponseModeDeclaration = true
            this.emit({
              type: 'notification',
              level: 'warning',
              message: 'Model did not declare its response mode; retrying with an explicit tool selection.',
            })
            return await this.callModel()
          }
        }
        turn.content = ''
        turn.metadata = { ...turn.metadata, thinking: undefined, internal: true, internalKind: 'response_mode' }
      }
      return turn
    } catch (error) {
      ...
      this.declaringResponseMode = false
      this.forceResponseModeDeclaration = false
    }
  }

  private declaringResponseMode = false
  private forceResponseModeDeclaration = false

  private needsResponseMode(): boolean {
    return Boolean(this.workExecution.getCurrentRunId() && !this.workExecution.getResponseMode())
  }
  ...
  private async callAnthropicAPI(
    ...
    if (cachedTools.length > 0) {
      ...
      // Forced tool choice cannot be combined with Anthropic thinking.
      if (this.needsResponseMode()) {
        delete requestBody.thinking
        delete requestBody.output_config
      }
      requestBody.tool_choice = this.needsResponseMode()
        ? { type: 'tool', name: RESPONSE_MODE_TOOL }
        : { type: 'auto' }
    }
  ...
  private async callOpenAICompatibleAPI(
    ...
    if (openaiTools.length > 0) {
      ...
      body.tool_choice = this.needsResponseMode()
        ? { type: 'function', function: { name: RESPONSE_MODE_TOOL } }
        : 'auto'
      ...
      body.parallel_tool_calls = !this.needsResponseMode()
    }
  ...
  private async callOpenAIResponsesAPI(
    ...
    if (responseTools.length > 0) {
      ...
      // Keep the tools prefix identical for declarations and normal requests.
      // The prompt and validated declaration still require one mode call first.
      body.tool_choice = this.needsResponseMode() && this.forceResponseModeDeclaration
        ? { type: 'function', name: RESPONSE_MODE_TOOL }
        : 'auto'
...
Support claims with inspected source text. Use known paths directly, or locate candidates with search_files/search_content and read the relevant ranges. State residual uncertainty plainly.
  ...
  private async executeToolCalls(toolCalls: ToolCall[]): Promise<ToolResult[]> {
    ...
    if (toolCalls.some(call => call.name === RESPONSE_MODE_TOOL)) {
      const mode = declaredResponseMode({ toolCalls } as AgentTurn)
      this.workExecution.setResponseMode(mode)
      this.emitWorkExecution()
      const result: ToolResult = {
        toolCallId: toolCalls[0].id,
        name: RESPONSE_MODE_TOOL,
        output: JSON.stringify({ mode }),
        isError: false,
      }
      this.emit({ type: 'tool:result', toolResult: result })
      return [result]
  ...
  private recordWorkflowStageResponse(workflowId: string, stage: string, response: string, workRunId: string): void {
    ...
    const userTurn = [...this.session.turns].reverse().find(turn => (
      turn.role === 'user' && (turn.metadata?.workRunId === workRunId || !turn.metadata?.workRunId)
    ))
    if (!userTurn) return
    ...
    switch (name) {
      ...
      case 'search_content': {
        ...
        const page = this.toolExecutor.searchContentPage
          ? result.data as import('@turboflux/contracts/toolExecutor').SearchContentPage
      ...
      case 'spawn_agent': {
        ...
        const def = getSubAgentDefinition(agentType, this.agentRegistry)
        if (!def) return `Error: unknown agent_type "${agentType}". Available: ${getAvailableAgentTypes(this.agentRegistry).join(', ')}.`
        if (!this.config.workspacePath) {
  ...
  private startSubAgentTask(
    ...
    if (policy) {
      if (!policy.allowedAgentTypes.includes(definition.id)) throw new Error(`Subagent type is not authorized for this automation run: ${definition.id}`)
      const tasks = this.subAgentTaskManager.listTasks().filter(task => !policy.runId || task.runtimeTask.metadata?.workRunId === policy.runId)
      if (tasks.length >= policy.maxSubtasks) throw new Error(`Automation run reached its ${policy.maxSubtasks}-subtask limit`)
      const active = tasks.filter(task => ['starting', 'running', 'stopping'].includes(task.runtimeTask.status)).length
      ...
      run: async ({ signal, recordEvent, taskId }) => {
        ...
        return runSubAgent({
          ...
          allowedTools: policy?.allowedTools.length
            ? (definition.allowedTools ?? policy.allowedTools).filter(tool => policy.allowedTools.includes(tool))
  ...
  private emit(event: AgentEventType): void {
    if (this.declaringResponseMode && event.type.startsWith('stream:') && event.type !== 'stream:usage') return
