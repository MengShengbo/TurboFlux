...
import type { SubAgentEvent, SubAgentEvidence, SubAgentDefinition } from '@turboflux/contracts/subAgentTypes'
import type { NativeReasoningConfig } from '@turboflux/contracts/agentTypes'
import type { ToolExecutor } from '@turboflux/contracts/toolExecutor'
import type { ModelCapabilities } from '@turboflux/models/config'
import { createTurboFluxRequestHeaders } from '@turboflux/models/clientIdentity'
import { toolsToOpenAIFormat } from '@turboflux/tools/toolRegistry'
...
import { loadAgentsFromDir, type LoadedAgent } from '@turboflux/extensions/agents/loader'
import type { SkillRuntime } from '@turboflux/extensions/skills/runtime'
import type { LoadedSkill } from '@turboflux/extensions/skills/loader'
import {
...
type BufferedModelResponse = Pick<Response, 'ok' | 'status' | 'headers' | 'text' | 'json'>

async function fetchWithTimeout(url: string, init: RequestInit, parentSignal?: AbortSignal, timeoutMs = 120_000): Promise<BufferedModelResponse> {
  ...
  try {
    response = await Promise.race([fetch(url, { ...init, signal: controller.signal }), interrupted])
    ...
    // Keep parsing inside the same deadline; later consumers only read memory.
    const json = response.ok ? JSON.parse(text) : undefined
    controller.signal.throwIfAborted()
    return { ok: response.ok, status: response.status, headers: response.headers,
      text: async () => text, json: async () => json }
  } finally {
    clearTimeout(timer)
...
function retryAfterMs(response: Pick<Response, 'headers'>): number {
  ...
  while (turn < turnLimit) {
    ...
    try {
      ...
      for (let protocolIndex = 0; protocolIndex < protocolCandidates.length; protocolIndex += 1) {
        ...
        let res: BufferedModelResponse | undefined
        let errorText = ''
    ...
    if (abortSignal?.aborted) return { ok: false, turns: turn, elapsedMs: Date.now() - startedAt, error: 'Aborted' }
