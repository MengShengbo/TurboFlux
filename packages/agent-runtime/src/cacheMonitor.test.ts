...
describe('CacheMonitor', () => {
  it('does not diagnose expiry from a long response when requests are adjacent', () => {
    const monitor = new CacheMonitor()
    monitor.recordPromptState(snapshot('stable system', 'gpt-6-astra'))
    monitor.checkCacheBreak(15104, 0, 15909, { requestStartedAt: 1000, responseReceivedAt: 402015 })
    monitor.recordPromptState(snapshot('stable system', 'gpt-6-astra'))
    const result = monitor.checkCacheBreak(10368, 0, 24272, { requestStartedAt: 402150, responseReceivedAt: 721168 })

    expect(result.broken).toBe(true)
    expect(result.likelyTtlExpiry).toBe(false)
    expect(result.reason).not.toContain('TTL')
    expect(result.requestTiming).toEqual({ idleMs: 135, durationMs: 319018 })
  })

  it('still reports possible expiry after an actual idle gap before dispatch', () => {
    const monitor = new CacheMonitor()
    monitor.recordPromptState(snapshot())
    monitor.checkCacheBreak(20000, 0, 25000, { requestStartedAt: 1000, responseReceivedAt: 2000 })
    monitor.recordPromptState(snapshot())
    const result = monitor.checkCacheBreak(10000, 0, 25000, { requestStartedAt: 362000, responseReceivedAt: 363000 })

    expect(result.likelyTtlExpiry).toBe(true)
    expect(result.requestTiming).toEqual({ idleMs: 360000, durationMs: 1000 })
  })
