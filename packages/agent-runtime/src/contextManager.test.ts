...
import type { ContextSegment } from '@turboflux/contracts/stateTypes'
...
describe('ContextManager', () => {
  ...
  it('tracks provider usage without local estimates', () => {
    ...
    expect(manager.getCurrentContextUsage()).toEqual({ source: 'unknown' })

    manager.updateCurrentContextUsage(42_000, 500, 30_000)

    expect(manager.getCurrentContextUsage()).toEqual({
      input: 42_000,
      output: 500,
      cached: 30_000,
      total: 42_500,
