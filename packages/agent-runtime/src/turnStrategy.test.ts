...
import type { AgentMode, AgentSession, AgentTurn, ToolResult } from '@turboflux/contracts/agentTypes'
...
describe('TurnStrategyPlanner', () => {
  ...
  it('does not let strategy hide read tools', () => {
    ...
    expect(names).toContain('search_content')
    expect(names).not.toContain('get_codemap')
    expect(names).not.toContain('search_symbols')
  })
