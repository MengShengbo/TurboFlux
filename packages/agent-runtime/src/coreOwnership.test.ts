...
describe('core reuse ownership boundary', () => {
  it('keeps reusable orchestration modules independent from UI frameworks', () => {
    const files = [
      'packages/agent-runtime/src/contextCompactionBoundary.ts',
      'packages/models/src/modelRequestOrchestrator.ts',
      'packages/agent-runtime/src/taskToolDispatcher.ts',
      'packages/agent-runtime/src/toolCallOrchestrator.ts',
    ]
  ...
  it('keeps the execution kernel independent from Desktop platform services', () => {
    const roots = readdirSync(join(process.cwd(), 'packages'), { withFileTypes: true }).filter(entry => entry.isDirectory()).map(entry => entry.name)
    ...
    roots.forEach(root => collect(join(process.cwd(), 'packages', root, 'src')))
    ...
    expect(openSourceConfig).toContain('"packages/*/src/**/*.ts"')
    const coreManifest = readFileSync(join(process.cwd(), 'packages', 'agent-core', 'package.json'), 'utf8')
    expect(coreManifest).not.toMatch(/desktop:|control-plane:|website:|electron/)
  })

  it('keeps product experience prompts in private adapters', () => {
    const workbenchRuntime = readFileSync(join(process.cwd(), 'packages/workbench/src/workbench/workbenchRuntime.ts'), 'utf8')
