import type { AgentRunInterruption } from '@turboflux/contracts/agentTypes'
...
export class AgentRunControl {
  ...
  pause(options: AgentRunPauseOptions = {}): boolean {
    if (!this.active || this.paused || this.runController?.signal.aborted) return false
    this.paused = true
  ...
  async runAcrossPause<T>(operation: () => Promise<T>, runSignal = this.getRunSignal()): Promise<T> {
    while (true) {
  ...
  raceWithStop<T>(operation: Promise<T>, runSignal = this.getRunSignal()): Promise<T> {
    ...
    if (runSignal.aborted) {
      // The caller has already started this promise; observe a late rejection.
      void operation.catch(() => undefined)
      return Promise.reject(this.createStopInterruption())
    }
