...
describe('AgentRunControl', () => {
  ...
  it('cannot pause or revive a stopped run before it finishes', () => {
    const control = new AgentRunControl()
    control.start()
    control.stop()
    ...
    expect(control.resume()).toBe(false)
    expect(control.getSnapshot()).toMatchObject({ paused: false, runAborted: true, operationAborted: true })
  })

  it('does not begin a retryable operation after stop', async () => {
    const control = new AgentRunControl()
    control.start()
    control.stop()
    let called = false

    await expect(control.runAcrossPause(async () => { called = true })).rejects.toMatchObject({ interruption: 'stop' })
