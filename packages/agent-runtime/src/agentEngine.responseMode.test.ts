...
describe('model response mode declaration', () => {
  it.each([1, 3])('counts repeated mode declarations toward a budget of %i tool rounds', async maxToolRounds => {
    const streamMessage = vi.fn(async (_url, _headers, _serialized, onLine) => {
      ...
      onLine(`data: ${JSON.stringify({ choices: [{ delta: declaring
        ? { tool_calls: [{ index: 0, id: `mode-${request}`, type: 'function', function: { name: 'set_response_mode', arguments: '{"mode":"task"}' } }] }
    ...
    const engine = new AgentEngine({ mode: 'vibe', approvalPolicy: 'full', gitEnabled: false, maxToolRounds }, { streamMessage } as unknown as ToolExecutor, state)
    const events: AgentEventType[] = []
    engine.subscribe(event => events.push(event))
    vi.spyOn(engine as any, 'prepareContextWindow').mockResolvedValue(undefined)
    try {
      await engine.run('Inspect')
      expect(streamMessage).toHaveBeenCalledTimes(maxToolRounds + 1)
