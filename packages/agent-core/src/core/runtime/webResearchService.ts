...
export class WebResearchService {
  ...
  private async validatePublicUrl(value: string): Promise<URL> {
    ...
    if (addresses.length === 0 || addresses.some(isPrivateAddress)) throw new Error('Local, private-network, and reserved addresses cannot be read')
    return url
  }
  ...
  private async readLimitedResponse(response: Response, maxBytes: number): Promise<string> {
    ...
    if (Number.isFinite(declared) && declared > maxBytes) throw new Error('The webpage response is too large and was not read')
    ...
    while (true) {
      ...
      if (total > maxBytes) {
        await reader.cancel()
        throw new Error('The webpage response exceeds the safe size limit')
      }
      chunks.push(value)
    }
