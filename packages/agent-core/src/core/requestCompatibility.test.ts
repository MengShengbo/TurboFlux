...
describe('request compatibility', () => {
  ...
  it('uses GPT-5.6 cache lifetime controls and removes them when unsupported', () => {
    const body: Record<string, unknown> = {}
    setOpenAIPromptCacheLifetime(body, 'gpt-5.6-sol')

    expect(body.prompt_cache_options).toEqual({ ttl: '30m' })
    expect(removeOpenAICompatibleRequestParam(body, 'prompt_cache_options.ttl')).toBe(true)
    expect(body).toEqual({})
  })
