...
export function linearTaskFlowItems(state: TaskFlowProjectionState, groupBoundary?: (node: TaskFlowNode) => string): LinearTaskFlowItem[] {
  ...
  for (const node of nodes) {
    if (node.kind === 'tool' && (node.toolName || node.content) === 'set_response_mode') continue
    if (node.kind === 'input') {
...
export function groupedToolStatus(item: Extract<LinearTaskFlowItem, { kind: 'tool-group' }>): TaskFlowNode['status'] {
  ...
  if (item.active && item.group !== 'repeat' && item.group !== 'retrieval') return 'running'
...
export function phaseTitle(node: Pick<TaskFlowNode, 'content' | 'status'>): string {
  ...
  if (normalized.includes('planning') || normalized.includes('next step')) return '正在处理请求'
  if (/running\s+\d+\s+tools?/.test(normalized) || normalized.includes('tool_running')) return '正在执行工具'
  if (normalized.includes('thinking')) return '正在处理请求'
  ...
  return node.content.trim() || '正在处理请求'
}
