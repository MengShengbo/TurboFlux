...
import { createProfileStorageLayout, ensureProfileStorageLayout } from '../profiles/profileStorageLayout'
import type { AutomationClaim } from '../automations/automationService'
...
describe('WorkbenchRuntime', () => {
  ...
  it('does not auto-present a workflow when a design plugin is mounted', async () => {
    ...
    const workflows: Array<import('../../shared/workflowSurfaceTypes').WorkflowRunContract | undefined> = []
  ...
  it('publishes partial task settlement without reporting false completion', async () => {
    ...
    try {
      ...
      expect(events).toContainEqual(expect.objectContaining({
        ...
        event: expect.objectContaining({ type: 'run.completed', payload: { outcome: 'partial' } }),
      }))
    } finally {
      await runtime.destroy()
    }
