...
import { copyToolResultDetails } from '../../shared/toolResultData'
...
import type { AnyAppendConversationEventV2Input, ConversationItemV2, ConversationRunV2, ConversationTranscriptProjectionV2, ConversationV2ItemStatus, ConversationV2RunStatus } from './conversationV2Types'
...
import type { WorkActivity, WorkExecutionSnapshot, WorkRun, WorkRunStatus, WorkStep, WorkStepStatus } from '../../shared/workExecutionTypes'
...
function itemForTurn(projection: ConversationTranscriptProjectionV2, turnId: string, kind: ConversationItemV2['kind']): ConversationItemV2 | undefined {
  return projection.items.find(item => item.turnId === turnId && item.kind === kind)
}
...
function turnFromProjection(projection: ConversationTranscriptProjectionV2, turnId: string): AgentTurn | null {
  const turn = projection.turns.find(candidate => candidate.id === turnId)
  if (!turn) return null
  const message = itemForTurn(projection, turnId, turn.role === 'assistant' ? 'assistant_message' : 'user_message')
  const toolCalls = projection.items.filter(item => item.turnId === turnId && item.kind === 'tool_call').map(item => {
  ...
  const toolResults = projection.items.filter(item => item.turnId === turnId && item.kind === 'tool_result').map(item => {
  ...
  return {
    ...
    ...((turn.status === 'interrupted' || turn.runId) ? { metadata: {
...
function workExecutionFromProjectionV2(projection: ConversationTranscriptProjectionV2): WorkExecutionSnapshot {
  ...
  const runs = projection.runs.map(run => {
    ...
    for (const call of items.filter((item): item is Extract<ConversationItemV2, { kind: 'tool_call' }> => item.kind === 'tool_call')) {
      const result = items.find((item): item is Extract<ConversationItemV2, { kind: 'tool_result' }> => (
        item.kind === 'tool_result' && item.payload.toolCallId === call.payload.toolCallId
      ))
...
function canonicalEventsFromProjectionV2(projection: ConversationTranscriptProjectionV2): AnyConversationEvent[] {
  const conversationId = projection.conversation?.id
  if (!conversationId) return []
  const fallbackOrder = projection.throughSeq + 1
  const timelineOrder = (
    type: string,
    coordinates: { runId?: string; turnId?: string; itemId?: string } = {},
    useLast = false,
  ): number => {
    const matches = projection.timeline.filter(entry => (
      entry.type === type
      && (!coordinates.runId || entry.runId === coordinates.runId)
      && (!coordinates.turnId || entry.turnId === coordinates.turnId)
      && (!coordinates.itemId || entry.itemId === coordinates.itemId)
    ))
    return (useLast ? matches.at(-1) : matches[0])?.seq ?? fallbackOrder
  }
  ...
  for (const turn of projection.turns) {
    const restoredTurn = turnFromProjection(projection, turn.id)
    if (!restoredTurn) continue
    add({
...
export function persistedConversationFromProjectionV2(
  ...
  const turns = projection.turns.map(turn => turnFromProjection(projection, turn.id)).filter((turn): turn is AgentTurn => Boolean(turn))
  return {
    id: conversation.id,
    ...
    turns,
    canonicalEvents: canonicalEventsFromProjectionV2(projection),
  ...
  appendCanonical(event: AnyConversationEvent, conversation: PersistedConversation): void {
    ...
    switch (event.type) {
      ...
      case 'run.state_changed':
        ...
        break
      case 'run.completed':
        if (common.runId) inputs.push({ ...common, eventId: canonicalEventId(event), type: 'run.completed', payload: { status: completedStatus(event.payload.outcome), completedAt: event.at, outcome: event.payload.error } })
        break
      case 'turn.started': {
      ...
      case 'turn.completed': {
        ...
        inputs.push({ ...common, eventId: canonicalEventId(event), turnId, type: 'turn.completed', payload: { completedAt: event.at, interrupted: turn.metadata?.interrupted } })
        break
      }
      ...
      case 'runtime.event': {
        ...
        } else if (event.payload.kind === 'subagent:progress' && payload) {
          const id = scopedId('subagent', String(payload.agentId))
        ...
        } else if (event.payload.kind === 'context:segment_created' && payload) {
        ...
        } else if (event.payload.kind === 'task:system' && payload) {
          const item: ConversationItemV2 = {
            schemaVersion: 1,
        ...
        } else if (event.payload.kind === 'active:task' && payload) {
        ...
        } else if (event.payload.kind === 'task:update' && payload) {
        ...
        } else if (event.payload.kind === 'work:execution' && payload) {
        ...
        } else if ((event.payload.kind === 'runtime-task:created' || event.payload.kind === 'runtime-task:updated' || event.payload.kind === 'runtime-task:finished') && payload) {
