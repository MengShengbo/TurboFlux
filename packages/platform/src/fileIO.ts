...
function shouldRemoveLock(lockPath: string, now: number): boolean {
  ...
  try {
    ...
    return age >= LOCK_STALE_AFTER_MS
  }
}
  ...
  while (descriptor === undefined) {
    try {
      ...
      try {
        ...
        writeFileSync(candidate, JSON.stringify({ pid: process.pid, token, createdAt: new Date().toISOString() }), 'utf-8')
        descriptor = candidate
        heldFileLocks.set(normalizedLockPath, { descriptor, depth: 1, token })
      } catch (error) {
        closeSync(candidate)
      ...
      if (now - startedAt >= timeoutMs) throw new Error(`Timed out waiting for file lock: ${normalizedLockPath}`, { cause: error })
      Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 25)
    }
  }
  const heldLock = heldFileLocks.get(normalizedLockPath)
  if (!heldLock) throw new Error(`File lock ownership was lost: ${normalizedLockPath}`)
  try {
    return callback()
  } finally {
    ...
    closeSync(descriptor)
    try {
      const current = JSON.parse(readFileSync(normalizedLockPath, 'utf-8')) as { token?: unknown }
      if (current.token === heldLock.token) rmSync(normalizedLockPath, { force: true })
    } catch {}
  }
}
...
export async function writeFileAtomic(filePath: string, content: string, options?: {
  /** Runs immediately before publication, inside the file lock. Must be synchronous. */
  beforeCommit?: () => void
  lockPath?: string
