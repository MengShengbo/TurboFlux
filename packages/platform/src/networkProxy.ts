...
let configuredProxy: NetworkProxyConfiguration | undefined
let configuredDispatcher: Dispatcher | undefined
const pinnedDispatchers = new Map<boolean, Dispatcher>()
...
export async function closeNetworkDispatcher(): Promise<void> {
  await Promise.all([getGlobalDispatcher().close(), ...[...pinnedDispatchers.values()].map(dispatcher => dispatcher.close())])
...
function bypassesProxy(url: URL, noProxy: string): boolean {
  ...
  return noProxy.split(/[,\s]+/).some(entry => {
    ...
    if (parts && Number(parts[2]) !== port) return false
    const host = (parts ? parts[1]! : entry).replace(/^\*?\./, '').toLowerCase()
    return hostname === host || hostname.endsWith(`.${host}`)
  })
}

/** Choose the configured proxy before DNS pinning rewrites the transport origin. */
export function getPinnedNetworkDispatcher(originalUrl: URL): Dispatcher {
  const current = getGlobalDispatcher()
  if (!configuredProxy || current !== configuredDispatcher) return current
  const bypass = bypassesProxy(originalUrl, configuredProxy.noProxy)
  let dispatcher = pinnedDispatchers.get(bypass)
  if (!dispatcher) {
    dispatcher = bypass ? new Agent() : new EnvHttpProxyAgent({
      httpProxy: configuredProxy.httpProxy,
      httpsProxy: configuredProxy.httpsProxy,
      noProxy: '',
    })
    pinnedDispatchers.set(bypass, dispatcher)
  }
  return dispatcher
}
