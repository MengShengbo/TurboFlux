import type { WorkRun, WorkStep } from '@turboflux/presentation'
import { presentWorkStages } from '@turboflux/presentation'
...
export function createWorkPlanDisclosure(
  ...
  title.textContent = variant === 'inline' ? '更新任务计划' : '任务列表'
  ...
  return {
    render(run) {
      if (!run || run.presentation !== 'work' || Object.keys(run.steps).length === 0) {
        if (renderedVersion !== 'empty') {
          const empty = document.createElement('section')
          empty.className = 'work-plan work-plan-empty'
          const title = document.createElement('strong')
          title.textContent = '任务列表'
          const detail = document.createElement('p')
          detail.textContent = '任务步骤和进度会显示在这里'
          empty.append(title, detail)
          host.replaceChildren(empty)
        }
        host.hidden = false
        renderedRunId = ''
        renderedVersion = 'empty'
        userCollapsed = null
        return
      }
