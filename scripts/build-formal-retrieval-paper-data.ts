...
function main(): void {
  ...
  writeJson(join(outputDirectory, 'dataset-slices.json'), datasetRows)
  writeCsv(join(outputDirectory, 'dataset-slices.csv'), datasetRows as unknown as Array<Record<string, unknown>>)
  writeJson(join(outputDirectory, 'language-slices.json'), languageRows)
  writeCsv(join(outputDirectory, 'language-slices.csv'), languageRows as unknown as Array<Record<string, unknown>>)
  writeJson(join(outputDirectory, 'category-slices.json'), categoryRows)
  writeCsv(join(outputDirectory, 'category-slices.csv'), categoryRows as unknown as Array<Record<string, unknown>>)
  writeJson(join(outputDirectory, 'failure-audit.json'), failureAuditRows)
  writeCsv(join(outputDirectory, 'failure-audit.csv'), failureAuditRows)
  writeJson(join(outputDirectory, 'latency-cdf.json'), latencyRows)
  writeCsv(join(outputDirectory, 'latency-cdf.csv'), latencyRows)
  const qualityCost = qualityCostRows(aggregateRows)
  writeJson(join(outputDirectory, 'quality-cost.json'), qualityCost)
  writeCsv(join(outputDirectory, 'quality-cost.csv'), qualityCost)
  writeJson(join(outputDirectory, 'repeat-stability.json'), repeatRows)
