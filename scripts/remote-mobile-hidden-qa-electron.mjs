...
async function main() {
  ...
  invariant(workspace.viewport.width === 390 && workspace.viewport.height === 844, `mobile viewport is not 390x844: ${workspace.viewport.width}x${workspace.viewport.height}`)
  invariant(workspace.document.width <= workspace.viewport.width, 'workspace overflows horizontally')
  invariant(workspace.hasFocus === false && !window.isFocused(), `hidden window received focus: document=${workspace.hasFocus}, window=${window.isFocused()}`)
  invariant(workspace.secureContext === true, 'localhost page is not a secure browser context')
  invariant(workspace.approvalVisibility?.width > 0 && workspace.approvalVisibility?.height >= 120, `approval card is not materially visible: ${workspace.approvalVisibility?.width}x${workspace.approvalVisibility?.height}`)
