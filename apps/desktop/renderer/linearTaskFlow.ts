...
import { deepSeekHarnessIcon } from './deepSeekHarnessIcons'
import { toolActivityIcon, toolActivitySummary } from './toolActivityPresentation'
import { createRetrievalGroupView, replaceRetrievalView } from './retrievalView'
import { createToolResultView } from './toolResultView'
import { taskRunSegmentIndex, taskRunStatusLabel, withTaskRunStatus, type TaskRunFlowItem } from './taskRunPresentation'
...
export type { LinearFlowGap, LinearTaskFlowItem, LinearToolGroupKind } from '@turboflux/agent-core/renderer'
...
function toolPresentation(node: TaskFlowNode, tool: LinearTaskFlowTool): {
  ...
  const summary = interruption || plan?.summary || browserDetail
    || (change ? `${change.path} · +${change.addedLines ?? 0} −${change.removedLines ?? 0}` : '')
    || toolActivitySummary(call, result)
  return { title, summary }
}

const retrievalVersions = new WeakMap<HTMLElement, string>()

function updateToolBody(host: HTMLElement, tool: LinearTaskFlowTool, status?: string): void {
...
function toolGroupSummary(item: Extract<LinearTaskFlowItem, { kind: 'tool-group' }>, status: TaskFlowNode['status'], firstSummary: string, options: LinearTaskFlowRendererOptions): string {
  if (item.group === 'retrieval') {
    if (item.nodes.length === 1) return firstSummary
    const resources = item.nodes.flatMap(node => options.resolveTool(node).result?.retrieval?.resources || [])
    const read = new Set(resources.filter(resource => resource.state === 'read').map(resource => resource.path)).size
    const found = new Set(resources.filter(resource => resource.kind === 'file' && resource.state !== 'read').map(resource => resource.path)).size
    const parts = [read ? `已读取 ${read} 个文件` : '', found ? `找到 ${found} 个候选文件` : ''].filter(Boolean)
    if (status === 'running') parts.push('进行中')
    if (status === 'paused') parts.push('已暂停')
    if (status === 'failed') parts.push('部分操作失败')
    if (status === 'cancelled' || status === 'interrupted') parts.push('已停止')
    return parts.join(' · ') || `${item.nodes.length} 次操作`
  }
...
function createToolGroupNode(
  ...
  const summary = document.createElement('span')
  summary.className = 'linear-disclosure-summary'
  summary.textContent = toolGroupSummary(item, status, firstPresentation.summary, options)
  ...
  if (item.group !== 'retrieval') for (const [index, node] of item.nodes.entries()) bodyInner.append(createToolGroupEntry(node, options, index))
  body.append(bodyInner)
  root.append(row, body)
  toggleDisclosure(root, false)
  row.onclick = () => {
...
function updateToolGroupNode(
  ...
  root.querySelector<HTMLElement>('.linear-disclosure-summary')!.textContent = toolGroupSummary(item, status, firstPresentation.summary, options)
  const row = root.querySelector<HTMLButtonElement>(':scope > .linear-tool-row')
  if (row) {
    row.onclick = () => {
      const expanded = !root.classList.contains('expanded')
...
export function createLinearTaskFlowRenderer(
  ...
  const updateStatusLabels = () => {
    for (const item of statusItems) {
      ...
      if (label) label.textContent = taskRunStatusLabel(item.run, item.segmentIndex)
    }
  }
  ...
  const render = (state: TaskFlowProjectionState, force = false): void => {
    ...
    const items = withTaskRunStatus(linearTaskFlowItems(state, node => (
