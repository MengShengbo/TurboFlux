...
export const IntroScene: React.FC = () => {
  ...
  return <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
    ...
    {!cut ? <div style={{position: 'absolute', inset: 0, transform: `scale(${push})`, transformOrigin: '960px 620px', filter: blur ? `blur(${blur}px)` : undefined}}>
      <div style={{position: 'absolute', left: 600, top: 164, opacity: ease(frame, 0, 20)}}><BrandMark size={52} /></div>
      <WindowFrame title="local shell · ~/studio/aurora" style={{position: 'absolute', left: 400, top: 300, width: 1120, height: 500}}>
        <div style={{padding: '46px 50px', fontFamily: mono}}>
          <div style={{color: C.dim, fontSize: 22}}>~/studio/aurora <span style={{color: C.info}}>git:(feature/auth-proof)</span></div>
          <div style={{display: 'flex', alignItems: 'center', marginTop: 38, fontSize: 32, color: C.white, whiteSpace: 'pre'}}>
          ...
          <div style={{marginTop: 68, color: C.dim, fontSize: 16, letterSpacing: 2}}>LOCAL-FIRST · PERMISSION-AWARE · EVIDENCE-DRIVEN</div>
        </div>
      </WindowFrame>
      <div style={{position: 'absolute', left: 0, right: 0, bottom: 88, textAlign: 'center', fontFamily: mono, color: C.muted, fontSize: 20, letterSpacing: 3, opacity: ease(frame, 34, 54)}}>一句指令。整个工程开始流动。</div>
  ...
  return <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
    ...
    <div style={{position: 'absolute', inset: border, borderRadius: 12, overflow: 'hidden'}}>
      ...
      <div style={{position: 'absolute', left: 170, bottom: 82, opacity: titleP, transform: `translateY(${24 * (1 - titleP)}px)`, padding: '18px 24px', background: 'rgba(5,5,5,.82)', borderLeft: `3px solid ${C.cyan}`}}>
        <div style={{fontFamily: mono, color: C.white, fontSize: 34, fontWeight: 800}}>本地工作台，不只是聊天窗口。</div>
        <div style={{fontFamily: mono, color: C.muted, fontSize: 18, marginTop: 9}}>模型、上下文、队列、终端与恢复状态，同一处持续运行。</div>
      </div>
    </div>
    {[
...
const agents = [
  ...
  {name: 'TEST', detail: '672 checks passed', color: C.green, x: 1370, y: 170},
  {name: 'TERMINAL', detail: 'detached · running', color: C.yellow, x: 1370, y: 590},
  {name: 'GIT', detail: 'diff structured', color: '#C084FC', x: 790, y: 760},
];
...
export const AgentsScene: React.FC = () => {
  ...
  return <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
    ...
    <div style={{position: 'absolute', left: 82, top: 72, fontFamily: mono, opacity: ease(frame, 8, 22)}}><div style={{color: C.cyan, fontSize: 16, letterSpacing: 4}}>02 / ORCHESTRATION</div><div style={{color: C.white, fontSize: 48, fontWeight: 800, marginTop: 12}}>不是等待。是并行推进。</div></div>
  </AbsoluteFill>;
};
...
export const PermissionScene: React.FC = () => {
  ...
  return <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
    ...
    <div style={{position: 'absolute', left: 0, right: 0, bottom: 82, textAlign: 'center', fontFamily: mono, opacity: ease(frame, 0, 18)}}><span style={{color: C.white, fontSize: 34, fontWeight: 800}}>速度有边界。</span><span style={{color: C.muted, fontSize: 22, marginLeft: 20}}>关键写入与命令始终由你决定。</span></div>
  </AbsoluteFill>;
};
...
const DiffLine: React.FC<{frame: number; cue: number; line: typeof diffLines[number]}> = ({frame, cue, line}) => {
  const {fps} = useVideoConfig();
  const p = spring({frame: frame - cue, fps, config: {damping: 16, stiffness: 145, mass: .7}});
  const press = interpolate(frame, [cue + 13, cue + 17, cue + 22], [0, 6, 0], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
  return <div style={{height: 62, display: 'flex', alignItems: 'center', transform: `translateY(${90 * (1 - p) + press}px)`, opacity: p, borderBottom: `1px solid ${C.lineSoft}`, background: line.prefix === '+' ? `${C.green}0a` : line.prefix === '-' ? `${C.red}0b` : 'transparent', padding: '0 22px', boxSizing: 'border-box', fontFamily: mono, fontSize: 17}}>
    ...
    <div style={{position: 'relative', height: '100%', display: 'grid', gridTemplateColumns: '480px 1fr', gap: 66}}>
      <div style={{display: 'flex', flexDirection: 'column', justifyContent: 'center', opacity: ease(frame, 0, 18)}}>
        ...
        <div style={{marginTop: 40, display: 'flex', flexDirection: 'column', gap: 12}}>
          <Badge color={C.green}>✓ 672 TESTS PASSED</Badge>
          <Badge color={C.info}>4 FILES · +18 / −4</Badge>
          <Badge color={C.cyan}>git diff --check · CLEAN</Badge>
        </div>
      </div>
      <WindowFrame title="git diff · feature/auth-proof" right={<Badge color={C.green}>REVIEW READY</Badge>} style={{height: 780, alignSelf: 'center'}}>
        ...
        <div>{diffLines.map((line, index) => <DiffLine key={`${line.prefix}-${index}`} frame={frame} cue={14 + index * 12} line={line} />)}</div>
        <div style={{height: 100, display: 'flex', alignItems: 'center', padding: '0 24px', gap: 18, fontFamily: mono, opacity: score, transform: `translateY(${18 * (1 - score)}px)`}}>
          ...
          <span style={{color: C.white, fontSize: 19}}>变更已验证，可审查，可恢复。</span>
        </div>
      </WindowFrame>
    </div>
  </AbsoluteFill>;
...
export const OutroScene: React.FC = () => {
  ...
  return <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
    ...
    <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', opacity: logoP, transform: `scale(${.92 + .08 * logoP})`}}>
      <BrandMark size={100} />
      <div style={{fontFamily: mono, color: C.white, fontSize: 28, marginTop: 44, letterSpacing: 1}}>Flow from intent to proof.</div>
      <div style={{fontFamily: mono, color: C.muted, fontSize: 16, marginTop: 20, letterSpacing: 4}}>LOCAL · ORCHESTRATED · REVIEWABLE</div>
    </AbsoluteFill>
  </AbsoluteFill>;
};
