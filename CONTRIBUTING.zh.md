...
GitHub Actions 还会验证 Linux、Windows 的领域包测试，macOS、Windows 的 Desktop 测试，以及 macOS、Windows、Linux 的打包与验收证据。必需的 **Quality gate** 只有在所有依赖任务成功后才会通过。`main` 的保护规则同样约束管理员，并禁止强制推送与删除。更新 `main` 前，维护者先推送临时验证分支，对同一提交运行 CI；通过后再推进 `main`，并删除临时分支。
